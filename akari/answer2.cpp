...
void printG(vector<vector<int>>& g);
    ...
    bool put_dot(vector<vector<int> >& board, int x, int y) {
        if (x == 3 && y == 6){}
//            printG(board);
        if (x < 0 || x >= n || y < 0 || y >= m) return true; // ??????true
        if (board[x][y] == BLUB) return false; // ???????????,?????dot?
    ...
    bool put_blub(vector<vector<int> >& board, int x, int y) {
        ...
        if (board[x][y] != EMPTY && board[x][y] != BLUB) return false; // ?????????????????
    ...
    bool solve_puzzle(Barrier* barr, vector<vector<int> >& g) {
        ...
        if (NULL != barr) {
            ...
            int r = barr->x, c = barr->y; // r???,c???
            bool handle = false; // ????????w
            
            copy_board(*cp_board, g); // ?????????g????
            switch (p->value) {
                case 4:
